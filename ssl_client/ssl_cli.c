...
int ssl_handshake(struct ssl_session *sess)
{
    int rc;
    int err;
    int events;
    ...
    SSL_set_mode(sess->ssl, SSL_MODE_AUTO_RETRY);

    events = POLLIN | POLLOUT;
    while ((rc = SSL_do_handshake(sess->ssl)) != 1) {
        err = SSL_get_error(sess->ssl, rc);
...
int ssl_conn(struct ssl_session *sess, const char *server, unsigned short port)
{
    int ret;

    sess->tcp_sock = sock_conn(server, port);
    if (sess->tcp_sock < 0) {
        ret = -1;
        goto err_out;
    }
    ...
    if (!SSL_set_fd(sess->ssl, sess->tcp_sock)) {
        ERR_print_errors_fp(stderr);
        ret = -3;
        goto err_out;
    }
...
void usage(char *prog)
{
    ...
    fprintf(stdout, "example: %s www.openssl.org 443 aFile\n", prog);
}
...
void run(void)
{
    ...
    if (ssl_conn(&sess, g_hostname, g_port))
        return;
